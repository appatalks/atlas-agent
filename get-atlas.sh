...
REPOSITORY="https://github.com/appatalks/atlas-agent.git"
INSTALL_DIR="${ATLAS_INSTALL_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/atlas-agent}"
PREVIOUS_INSTALL_DIR="${XDG_DATA_HOME:-$HOME/.local/share}/atlas-live-agentic-support"
LEGACY_INSTALL_DIR="${ATSLA_INSTALL_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/atsla-support-live-agent}"

usage() {
  cat <<'EOF'
Usage: curl -fsSL https://raw.githubusercontent.com/appatalks/atlas-agent/main/get-atlas.sh | bash
...
for previous_dir in "$PREVIOUS_INSTALL_DIR" "$LEGACY_INSTALL_DIR"; do
  if [[ ! -e "$INSTALL_DIR" && -d "$previous_dir/.git" ]]; then
    echo "Migrating ATLAS installation to $INSTALL_DIR"
